...
class spherical_sea_level(object):
    ...
    def __init__(self,grid):
        ...
        self.sdelI=np.zeros((len(grid.time_step)-1,3))
        self.sdelS=0 #: coeff
        self.delS=0 #: coeff
        self.delLa_prev=0
    ...
    def reset(self):
        n=self.TO.coeff.shape[0]
        t=self.sdelI.shape[0]
        self.TO.prev=np.zeros(n)+1j*0
        self.delLa_prev=np.zeros(n)+1j*0
        self.delS_prev=np.zeros(n)+1j*0
        self.sdelI=np.zeros((t,3))+1j*0
        self.sdelm=np.zeros((t,3))+1j*0
        self.sdelL=np.zeros((t,n))+1j*0
        self.sdelLa=np.zeros((t,n))+1j*0